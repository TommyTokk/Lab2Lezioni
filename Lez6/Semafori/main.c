...
/*== SEMAFORI ==
I semafori sono un meccanismo di sincronizzazione
...
int main(int argc,char *argv[])
{
  ...
  // ---- creo il semaforo
  sem_t *sem_a0 = xsem_open(Nome,O_CREAT|O_EXCL,0666,1,__LINE__, __FILE__);
  xsem_unlink(Nome,__LINE__, __FILE__); // distrugge sem quando finito	
  // ---- creo il secondo semaforo
  sem_t *sem_finito = xsem_open(Nome2,O_CREAT|O_EXCL,0666,0,__LINE__, __FILE__);
    ...
    if(pid==0) { //processo figlio
      ...
      for(int j=start;j<end;j++)
        if(primo(j)) {
          ...
          /*
          L'operazione di wait ha un doppio funzionamento:
          1) Decrementa di 1 la variabile del semaforo
          2) Se la variabile del semaforo è 0 sospende il processo chiamante
          */
          a[0] += 1;
          // riporta il semaforo a 1
					xsem_post(sem_a0,__LINE__, __FILE__);
          /*
          L'operazione di post ha il compito di incrementare la variabile associata
          al semaforo
          */	
        }
      ...
      // segnala al processo padre che questo processo ha finito 
			...
			// unmap memoria condivisa e chiusura semafori
