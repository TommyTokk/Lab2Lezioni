...
Persona *creaPersona(char *nome, char *cognome, int eta){
  Persona *p = malloc(sizeof(*p));
  p -> nome = strdup(nome);
...
Persona *rimuoviPersona(Persona *lis){
  if(!lis) return NULL;
  if(lis -> eta == 20){
    Persona *tmp = lis -> next;
    personaDistruggi(lis);
    return rimuoviPersona(tmp);
  }else{
    lis -> next = rimuoviPersona(lis -> next);
    return lis;
  }
}

void personaDistruggi(Persona *p){
  free(p -> nome);
  free(p -> cognome);
  free(p);
}

Persona *listaPersone(Persona *p, Persona *lis){
  if(!lis){
    return p;
  }
  p -> next = lis;
...
Persona *listaPersoneOrdina(Persona *p, Persona *lis){
  if(!lis){
    return p;
  }
  if(p -> eta < lis -> eta){
    ...
    lis -> next = listaPersoneOrdina(p, lis -> next);
    return lis;
  }
}

void personaStampa(Persona *p){
  printf("Nome: %s, Cognome: %s, Età: %d\n", p -> nome, p -> cognome, p -> eta);
}

void listaStampa(Persona *lis){
  /*while(lis != NULL){
    personaStampa(lis);
    lis = lis -> next;
  }*/
  if(lis != NULL){
    personaStampa(lis);
    listaStampa(lis -> next);
  }
}

void listaDistruggi(Persona *lis){
  if(lis != NULL){
    listaDistruggi(lis -> next);
    personaDistruggi(lis);
  }
}
